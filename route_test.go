...
func TestFallback(t *testing.T) {
	...
	mockConfig.EXPECT().GetInt("http.drivers.chi.body_limit", 4096).Return(4096).Once()
	...
	chi.Fallback(func(ctx contractshttp.Context) contractshttp.Response {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(host string, port string) error {
				...
				mockConfig.EXPECT().GetString("http.host").Return(host).Once()
				mockConfig.EXPECT().GetString("http.port").Return(port).Once()
				mockConfig.EXPECT().GetInt("http.drivers.chi.header_limit", 4096).Return(4096).Once()
...
func TestNewRoute(t *testing.T) {
	...
	defaultTemplate := new(template.Template)

	tests := []struct {
		name             string
		parameters       map[string]any
		setup            func()
		expectHTMLRender *template.Template
		expectError      error
	}{
		{
		...
		},
		{
			...
			setup: func() {
				mockConfig.EXPECT().Get("http.drivers.chi.template").Return(defaultTemplate).Once()
			},
			expectHTMLRender: defaultTemplate,
		},
		{
			...
			parameters: map[string]any{"driver": "gin"},
			setup: func() {
				mockConfig.EXPECT().Get("http.drivers.chi.template").Return(func() (*template.Template, error) {
