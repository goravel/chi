...
func TestGroup(t *testing.T) {
	...
	beforeEach := func() {
		...
		mockConfig.On("GetInt", "http.drivers.chi.body_limit", 4096).Return(4096).Once()
	...
	tests := []struct {
	...
	}{
		{
			name: "Get",
			setup: func(req *http.Request) {
				chi.Get("/input/{id}", func(ctx contractshttp.Context) contractshttp.Response {
					return ctx.Response().Json(http.StatusOK, contractshttp.Json{
						"id": ctx.Request().Input("id"),
					})
				})
		...
		},
		{
			name: "Post",
			setup: func(req *http.Request) {
				chi.Post("/input/{id}", func(ctx contractshttp.Context) contractshttp.Response {
		...
		},
		{
			name: "Put",
			setup: func(req *http.Request) {
				chi.Put("/input/{id}", func(ctx contractshttp.Context) contractshttp.Response {
					return ctx.Response().Success().Json(contractshttp.Json{
						"id": ctx.Request().Input("id"),
					})
				})
		...
		},
		{
			name: "Delete",
			setup: func(req *http.Request) {
				chi.Delete("/input/{id}", func(ctx contractshttp.Context) contractshttp.Response {
					return ctx.Response().Success().Json(contractshttp.Json{
						"id": ctx.Request().Input("id"),
					})
				})
		...
		},
		{
			name: "Options",
			setup: func(req *http.Request) {
				chi.Options("/input/{id}", func(ctx contractshttp.Context) contractshttp.Response {
					return ctx.Response().Success().Json(contractshttp.Json{
						"id": ctx.Request().Input("id"),
					})
				})
		...
		},
		{
			name: "Patch",
			setup: func(req *http.Request) {
				chi.Patch("/input/{id}", func(ctx contractshttp.Context) contractshttp.Response {
					return ctx.Response().Success().Json(contractshttp.Json{
						"id": ctx.Request().Input("id"),
					})
				})
		...
		},
		{
			name: "Any Get",
			setup: func(req *http.Request) {
				chi.Any("/any/{id}", func(ctx contractshttp.Context) contractshttp.Response {
					return ctx.Response().Success().Json(contractshttp.Json{
						"id": ctx.Request().Input("id"),
					})
				})
		...
		},
		{
			...
			setup: func(req *http.Request) {
				...
				chi.GlobalMiddleware(func(ctx contractshttp.Context) {
					ctx.WithValue("action", "index")
					ctx.Request().Next()
				})
				chi.Resource("/resource", resource)
			},
			method:     "GET",
			url:        "/resource",
			expectCode: http.StatusOK,
		...
		},
		{
			name: "StaticFile",
			setup: func(req *http.Request) {
				chi.StaticFile("static-file", "./README.md")
			},
			method:     "GET",
			url:        "/static-file",
			expectCode: http.StatusOK,
		},
		{
			name: "StaticFS",
			setup: func(req *http.Request) {
				chi.StaticFS("static-fs", http.Dir("./"))
			},
			method:     "GET",
			url:        "/static-fs",
		...
		},
		{
			...
			setup: func(req *http.Request) {
				chi.Middleware(abortMiddleware()).Get("/middleware/{id}", func(ctx contractshttp.Context) contractshttp.Response {
					return ctx.Response().Success().Json(contractshttp.Json{
						"id": ctx.Request().Input("id"),
					})
				})
		...
		},
		{
			...
			setup: func(req *http.Request) {
				chi.Middleware(contextMiddleware(), contextMiddleware1()).Get("/middlewares/{id}", func(ctx contractshttp.Context) contractshttp.Response {
					return ctx.Response().Success().Json(contractshttp.Json{
						"id":   ctx.Request().Input("id"),
						"ctx":  ctx.Value("ctx"),
						"ctx1": ctx.Value("ctx1"),
		...
		},
		{
			...
			setup: func(req *http.Request) {
				chi.Prefix("prefix1").Prefix("prefix2").Get("input/{id}", func(ctx contractshttp.Context) contractshttp.Response {
					return ctx.Response().Success().Json(contractshttp.Json{
						"id": ctx.Request().Input("id"),
					})
				})
		...
		},
		{
			...
			setup: func(req *http.Request) {
				chi.Prefix("group1").Middleware(contextMiddleware()).Group(func(route1 route.Router) {
		...
		},
		{
			...
			setup: func(req *http.Request) {
				...
				chi.Get("/global-middleware", func(ctx contractshttp.Context) contractshttp.Response {
					return ctx.Response().Json(http.StatusOK, contractshttp.Json{
		...
		},
		{
			...
			setup: func(req *http.Request) {
				chi.Prefix("conflict").Group(func(route1 route.Router) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			chi, err = NewRoute(mockConfig, nil)
			assert.Nil(t, err)
