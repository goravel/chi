...
func NewGroup(config config.Config, instance chi.Router, prefix string, originMiddlewares []httpcontract.Middleware, lastMiddlewares []httpcontract.Middleware) route.Router {
	return &Group{
		config:            config,
		instance:          instance,
		originPrefix:      prefix,
...
func (r *Group) Any(relativePath string, handler httpcontract.HandlerFunc) {
	r.getRoutesWithMiddlewares().Handle(relativePath, handlerToChiHandler(handler))
	r.clearMiddlewares()
}

func (r *Group) Get(relativePath string, handler httpcontract.HandlerFunc) {
	r.getRoutesWithMiddlewares().Get(relativePath, handlerToChiHandler(handler))
	r.clearMiddlewares()
}

func (r *Group) Post(relativePath string, handler httpcontract.HandlerFunc) {
	r.getRoutesWithMiddlewares().Post(relativePath, handlerToChiHandler(handler))
	r.clearMiddlewares()
}

func (r *Group) Delete(relativePath string, handler httpcontract.HandlerFunc) {
	r.getRoutesWithMiddlewares().Delete(relativePath, handlerToChiHandler(handler))
	r.clearMiddlewares()
}

func (r *Group) Patch(relativePath string, handler httpcontract.HandlerFunc) {
	r.getRoutesWithMiddlewares().Patch(relativePath, handlerToChiHandler(handler))
	r.clearMiddlewares()
}

func (r *Group) Put(relativePath string, handler httpcontract.HandlerFunc) {
	r.getRoutesWithMiddlewares().Put(relativePath, handlerToChiHandler(handler))
	r.clearMiddlewares()
}

func (r *Group) Options(relativePath string, handler httpcontract.HandlerFunc) {
	r.getRoutesWithMiddlewares().Options(relativePath, handlerToChiHandler(handler))
	r.clearMiddlewares()
}

func (r *Group) Resource(relativePath string, controller httpcontract.ResourceController) {
	r.getRoutesWithMiddlewares().Get(relativePath, handlerToChiHandler(controller.Index))
	r.getRoutesWithMiddlewares().Post(relativePath, handlerToChiHandler(controller.Store))
	r.getRoutesWithMiddlewares().Get(relativePath+"/{id}", handlerToChiHandler(controller.Show))
	r.getRoutesWithMiddlewares().Put(relativePath+"/{id}", handlerToChiHandler(controller.Update))
	r.getRoutesWithMiddlewares().Patch(relativePath+"/{id}", handlerToChiHandler(controller.Update))
	r.getRoutesWithMiddlewares().Delete(relativePath+"/{id}", handlerToChiHandler(controller.Destroy))
	r.clearMiddlewares()
}
...
func (r *Group) StaticFile(relativePath, filepath string) {
	...
	r.getRoutesWithMiddlewares().Get(relativePath, handlerToChiHandler(handler))
	r.getRoutesWithMiddlewares().Head(relativePath, handlerToChiHandler(handler))
	r.clearMiddlewares()
}
...
func (r *Group) getRoutesWithMiddlewares() chi.Router {
	...
	r.prefix = ""
	group := r.instance.Route(prefix, func(r chi.Router) {}) // We don't need to add any handler here
	...
	ginLastMiddlewares := middlewaresToChiHandlers(r.lastMiddlewares)
